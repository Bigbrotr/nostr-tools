...
from nostr_tools import (
    Client, Relay, Filter, Event,
    generate_keypair, generate_event,
    fetch_events, stream_events, compute_relay_metadata,
    fetch_nip11, check_connectivity,
        ...
        async with client:
            ...
            async for event in stream_events(client, filter):
                ...
                if event_count % 5 == 0:
                    ...
                    if event.tags:
                        tag_types = list(set(tag[0] for tag in event.tags if tag))
        ...
        async with client:
            ...
            for i, event in enumerate(metadata_events[:3], 1):
                try:
                    ...
                    print(f"   {i}. Invalid metadata ({event.pubkey[:16]}...)")
...
async def error_handling_example():
    ...
    try:
        malformed_relay = Relay("not-a-valid-url")
    ...
    for relay_url in test_relays:
        ...
        try:
            ...
            if nip11_data:
                ...
                print(f"    Software: {nip11_data.get('software', 'N/A')} {nip11_data.get('version', '')}")
